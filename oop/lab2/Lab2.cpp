...
// 	Сделал
	...
	MyString ss = s; // Создан новый объект типа MyString, таким образом, вызывается конструктор копирования
	...
	Vector::PrintCount();	// Вызываем без помощи объекта (12)
	
	Vector *pv = new Vector; // (13)
