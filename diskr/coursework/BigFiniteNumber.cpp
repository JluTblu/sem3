...
std::string BigFiniteNumber::pad_left(std::string num_str, int target_len) const {
    if (num_str.length() >= static_cast<size_t>(target_len)) return num_str;
...
void BigFiniteNumber::check_overflow() const {
    if (value.length() > static_cast<size_t>(MAX_DIGITS)) {
        throw std::overflow_error("Overflow: number exceeds " + std::to_string(MAX_DIGITS) + " digits");
    }
}
...
BigFiniteNumber BigFiniteNumber::power(const BigFiniteNumber& exp) const {
    if (exp.is_negative) {
        throw std::invalid_argument("Negative exponents not supported");
    }
    
    if (exp.value == std::string(1, get_additive_unit())) {
        return BigFiniteNumber::from_internal_string(std::string(1, get_multiplicative_unit()), false);
    }
    ...
    BigFiniteNumber result = BigFiniteNumber::from_internal_string(std::string(1, get_multiplicative_unit()), false);
    BigFiniteNumber counter = BigFiniteNumber::from_internal_string(std::string(1, get_additive_unit()), false);
    
    while (counter != exp) {
        result = result * (*this);
        counter = counter + BigFiniteNumber::from_internal_string(std::string(1, get_multiplicative_unit()), false);
    }
    
    return result;
}

BigFiniteNumber BigFiniteNumber::gcd(const BigFiniteNumber& a, const BigFiniteNumber& b) {
    BigFiniteNumber a_abs = BigFiniteNumber::from_internal_string(a.value, false);
    BigFiniteNumber b_abs = BigFiniteNumber::from_internal_string(b.value, false);
    
    while (b_abs.value != std::string(1, get_additive_unit())) {
        auto [q, r] = a_abs.divide(b_abs);
        a_abs = b_abs;
        b_abs = r;
    }
...
BigFiniteNumber BigFiniteNumber::lcm(const BigFiniteNumber& a, const BigFiniteNumber& b) {
    BigFiniteNumber a_abs = BigFiniteNumber::from_internal_string(a.value, false);
    BigFiniteNumber b_abs = BigFiniteNumber::from_internal_string(b.value, false);
    
    if (a_abs.value == std::string(1, get_additive_unit()) || b_abs.value == std::string(1, get_additive_unit())) {
        return BigFiniteNumber::from_internal_string(std::string(1, get_additive_unit()), false);
    }
    
    BigFiniteNumber gcd_val = gcd(a_abs, b_abs);
    BigFiniteNumber product = a_abs * b_abs;
    auto [quotient, remainder] = product.divide(gcd_val);
